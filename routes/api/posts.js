...
router.delete('/comment/:id/:comment_id', passport.authenticate('jwt', {
  ...
  Post.findById(req.params.id)
    .then(post => {
      ...
      // Check if the comment belongs to current user, only allow users to delete their own comments
      const c = post.comments.filter(comment => comment._id.toString() === req.params.comment_id);
      if (c[0].user.toString() !== req.user.id) {
        return res.status(401).json({
          notuserscomment: 'You may only delete your own comments.'
        });
      }
